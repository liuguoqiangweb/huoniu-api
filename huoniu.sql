...
#插入数据
INSERT INTO hn_settings VALUES
(NULL, '火牛', 'http://127.0.0.1:8090', 'http://127.0.0.1:8091', 'http://127.0.0.1:8092', '京ICP备12003709号-3', 'Copyright © 北京达内金桥科技有限公司版权所有');

#创建数据表-桌台信息表hn_table
CREATE TABLE hn_table(
    tid INT PRIMARY KEY AUTO_INCREMENT,                        #桌台编号
    tname VARCHAR(64),                          #桌台昵称，允许为空
    type VARCHAR(16),                           #桌台类型，如3-4人桌
    state INT                                   #当前状态，1-空闲，2-预定，3-占用，0-其他
);

#插入数据
INSERT INTO hn_table VALUES
(1, '金镶玉', '2人桌', 1),
(2, '玉如意', '2人桌', 1),
(3, '齐天寿', '6人桌', 3),
(5, '福临门', '4人桌', 2),
...
#创建数据表-桌台预定信息表hn_reservation
CREATE TABLE hn_reservation(
    rid INT PRIMARY KEY AUTO_INCREMENT,                        #信息编号
    contacName VARCHAR(64),                     #联系人姓名
    phone VARCHAR(16),                          #联系电话
    contacTime BIGINT,                          #联系时间
    dinnerTime BIGINT,                          #预约的用餐时间
    tableId INT,
    FOREIGN KEY(tableId) REFERENCES hn_table(tid)
);

#插入数据
INSERT INTO hn_reservation VALUES
...
#创建数据表-菜品分类表hn_category
CREATE TABLE hn_category(
    cid INT PRIMARY KEY AUTO_INCREMENT,                        #类别编号
    cname VARCHAR(32)                           #类别名称
);

#插入数据
INSERT INTO hn_category VALUES
(1, '肉类'),
(2, '海鲜河鲜'),
(3, '丸滑类'),
(4, '蔬菜豆制品'),
...
#创建数据表-菜品信息表hn_dish
CREATE TABLE hn_dish(
    did INT PRIMARY KEY AUTO_INCREMENT,                        #菜品编号，起始值为10000
    title VARCHAR(32),                          #菜品名称/标题
    imgUrl VARCHAR(128),                        #图片地址
    price DECIMAL(6,2),                         #价格
    detail VARCHAR(128),                        #详细描述信息
    categoryId INT,                             #所属类别的编号，外键，参考类别cid
    FOREIGN KEY(categoryId) REFERENCES hn_category(cid)
);

#插入数据
INSERT INTO hn_dish VALUES
...
#创建数据表-订单表hn_order_detail
CREATE TABLE hn_order_detail(
    did INT PRIMARY KEY AUTO_INCREMENT,                        #订单编号
    dishId INT,                                 #菜品编号
    dishCount INT,                              #菜品数量
    customerName VARCHAR(64),                   #点餐用户的称呼
    orderId INT,                                #订单编号，指明所属订单
    FOREIGN KEY(dishId) REFERENCES hn_dish(did),
    FOREIGN KEY(orderId) REFERENCES hn_order(oid)
);
#插入数据
INSERT INTO hn_order_detail VALUES
(NULL, '50', '2', '丁丁', '100000'),
(NULL, '24', '2', '丁丁', '100001'),
(NULL, '37', '1', '当当', '100002'),
(NULL, '18', '3', '豆豆', '100003'),
